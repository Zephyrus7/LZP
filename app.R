...
server <- function(input, output, session) {
  ...
  observeEvent(input$dark_mode_switch, {
    ...
    if (isTRUE(input$dark_mode_switch)) { shinyjs::addClass(selector = "body", class = "dark-mode"); theme_reactive("dark") } else { shinyjs::removeClass(selector = "body", class = "dark-mode"); theme_reactive("light") }
  })
  ...
  observeEvent(input$analiz_baslat, {
    ...
    if (input$analiz_modu == "statik") {
      ...
      if (input$statik_veri_secimi == 'tarih_sec') {
        req(input$tarih_araligi); local_start_date <- as.Date(input$tarih_araligi[1]); local_end_date <- as.Date(input$tarih_araligi[2]); db_min_date <- as.Date(db_date_range$min_date); db_max_date <- as.Date(db_date_range$max_date)
        if (local_start_date > db_max_date || local_end_date < db_min_date) { showModal(modalDialog(title = "Geçersiz Tarih Aralığı", p("Seçtiğiniz tarih aralığında veritabanında hiç veri bulunmamaktadır."), p(strong("Veritabanındaki mevcut veri aralığı:")), p(paste(format(db_min_date, "%d %B %Y"), "-", format(db_max_date, "%d %B %Y"))), footer = modalButton("Anladım"))); return() }
    ...
    if (!is.null(rv$data)) {
      ...
      if (rv$tip == "B2C") { updateNavbarPage(session, "main_navbar", selected = "Ağırlık Simülatörü") } else if (rv$tip == "B2B") { updateNavbarPage(session, "main_navbar", selected = "Kargo Firması Karnesi") }; download_tab_value <- "download_tab"; appendTab(inputId = "main_navbar", tabPanel(title = "Veri İndir", value = download_tab_value, icon = icon("download"), sidebarLayout(sidebarPanel(h4("İndirme Seçenekleri"), uiOutput(ns("download_options_ui")), hr(), downloadButton(ns("download_data_button"), "Seçilen Verileri İndir (.xlsx)", class="btn-success btn-block")), mainPanel(h3("Veri Raporlarını İndirin"), p("Sol taraftaki menüden indirmek istediğiniz raporları seçin."))))); rv$active_tabs <- c(rv$active_tabs, download_tab_value)
    }
  })
  ...
  output$download_options_ui <- renderUI({
    ...
    if(rv$tip == "B2C" && isTruthy(b2c_server_result$marka_data())) { choices_list <- c(choices_list, "Seçili Marka Analizi" = "marka_analizi") }
  ...
  output$download_data_button <- downloadHandler(
    ...
    content = function(file) {
      ...
      if(rv$tip == "B2C") {
        ...
        if ("marka_analizi" %in% input$download_choices) {
          marka_df <- b2c_server_result$marka_data()
          if(isTruthy(marka_df)) {
            secilen_marka_adi <- b2c_server_result$secilen_marka()
            sayfa_adi <- str_replace_all(str_sub(paste0("Marka_", secilen_marka_adi), 1, 31), "[^A-Za-z0-9_]", "_")
            list_of_datasets[[sayfa_adi]] <- marka_df %>% select(`Kargo Firması` = kargo_turu, `Hacim Ayarlı Skor` = Bayes_EPS, `Ham Skor` = Ham_EPS, `Ortalama Teslim Süresi` = ortalama_teslim_suresi, `Başarı Oranı` = dinamik_basari_orani, `Şikayet Oranı` = sikayet_orani_yuzde, `Toplam Gönderi` = toplam_gonderi_sayisi)
          }
        }
      } else {
        if ("b2b_main" %in% input$download_choices) list_of_datasets[["B2B_Ana_Veri"]] <- rv$data$main_data
      }
      if(length(list_of_datasets) > 0) { write.xlsx(list_of_datasets, file, asTable = TRUE, headerStyle = createStyle(textDecoration = "bold", fgFill = "#DDEBF7", halign = "center")) } else { showNotification("İndirilecek seçili bir rapor bulunamadı.", type = "warning"); file.create(file) }
    }
  )
}
