...
# =========================================================================
# YENİLİK: "Dinamik Karşılaştırma" butonu artık "Analizi Başlat" butonu
#          ile aynı animasyon mantığını kullanmaktadır. İşlem sırasında
#          dolgu efekti gösterir, metin değiştirir ve tıklanamaz hale gelir.
# =========================================================================

register_b2c_observers <- function(input, output, session, karsilastirma_verisi, db_pool_static, b2b_turleri) {
  
  ns <- session$ns
  
  # --- Firma Karnesi Sekmesi UI Yönetimi (Değişiklik Yok) ---
  ...
  # --- Dinamik Karşılaştırma Butonu (TAMAMEN YENİLENDİ) ---
  observeEvent(input$karsilastir_button, {
    req(input$ana_donem_secimi, input$karsilastirma_donem_secimi)
    
    # 1. GÜVENLİK AĞI: İşlem hata verse bile butonun eski haline dönmesini garantile
    on.exit({
      # Orijinal metni ve ikonu geri yükle
      shinyjs::html(id = "karsilastir_progress_text", html = 'Verileri Karşılaştır <i class="fa fa-exchange-alt" role="presentation" aria-label="exchange-alt icon"></i>')
      # Yüklenme animasyonunu kaldır
      shinyjs::removeClass(id = "karsilastir_container", class = "btn-loading")
      # Butonu tekrar tıklanabilir yap
      shinyjs::runjs(sprintf("$('#%s').css('pointer-events', 'auto');", ns("karsilastir_container")))
      # Dolgu efektini %100 yapıp kısa bir süre sonra sıfırla
      shinyjs::runjs(sprintf("$('#%s').css('width', '100%%');", ns("karsilastir_progress_fill")))
      Sys.sleep(0.5)
      shinyjs::runjs(sprintf("$('#%s').css('width', '0%%');", ns("karsilastir_progress_fill")))
    })
    
    # 2. ANINDA GERİ BİLDİRİM: Butonu "Meşgul" durumuna getir
    # Butonu tıklanamaz yap
    shinyjs::runjs(sprintf("$('#%s').css('pointer-events', 'none');", ns("karsilastir_container")))
    # Yüklenme animasyonunu (shimmer) ekle
    shinyjs::addClass(id = "karsilastir_container", class = "btn-loading")
    # Metni "Başlatılıyor..." olarak değiştir
    shinyjs::html(id = "karsilastir_progress_text", html = "Başlatılıyor...")
    # Dolgu efektini sıfırla
    shinyjs::runjs(sprintf("$('#%s').css('width', '0%%');", ns("karsilastir_progress_fill")))
    
    # 3. İŞLEM SIRASINDA GERİ BİLDİRİM: Dolgu efektini ilerlet
    # (Bu basit bir simülasyondur, daha karmaşık progress bar için `analiz_et_ve_skorla`'ya progress_updater eklenmelidir)
    shinyjs::html(id = "karsilastir_progress_text", html = "Veriler Çekiliyor...")
    shinyjs::runjs(sprintf("$('#%s').css('width', '25%%');", ns("karsilastir_progress_fill")))
    
    # 4. AĞIR HESAPLAMA: Veri çekme ve işleme mantığı
    ...
    # Dolgu efektini ilerlet
    shinyjs::html(id = "karsilastir_progress_text", html = "Hesaplanıyor...")
    shinyjs::runjs(sprintf("$('#%s').css('width', '65%%');", ns("karsilastir_progress_fill")))
    ...
    if (is.null(ana_ozet) || is.null(karsilastirma_ozet)) {
      ...
      # `return()` komutu on.exit bloğunu tetikler, bu nedenle buton düzelir.
      return()
    }
    
    # Dolgu efektini ilerlet
    shinyjs::html(id = "karsilastir_progress_text", html = "Tamamlanıyor...")
    shinyjs::runjs(sprintf("$('#%s').css('width', '90%%');", ns("karsilastir_progress_fill")))
    ...
    # İşlem bittiğinde dolgu efektini %100 yap. `on.exit` gerisini halledecektir.
    shinyjs::runjs(sprintf("$('#%s').css('width', '100%%');", ns("karsilastir_progress_fill")))
  })
  
  # --- Tablo Satırlarına Tıklama Olayları (Modal Pencere Açanlar) (Değişiklik Yok) ---
  ...
  observeEvent(input$marka_analizi_tablosu_rows_selected, {
    ...
    bolge_adi <- if (input$marka_analizi_secilen_il != "all_cities") {
      ...
      } else { str_to_upper(input$marka_analizi_secilen_il, "tr") }
    } else { "Tüm Türkiye" }
