...
# NOT: Bu versiyon, animasyonları sadece "Firma Karnesi" sekmesinde
#      tutarak, diğer sekmeleri orijinal, animasyonsuz hallerine döndürür.
# =========================================================================

# --- Firma Karnesi Sekmesi UI Yönetimi ---
# Bu sekme, animasyonlu yapısını koruyor.
...
observeEvent(firma_karne_filtrelenmis_veri(), {
  ...
  data_for_plot <- firma_karne_filtrelenmis_veri() %>% filter(toplam_gonderi_sayisi >= min_hacim)
...
observeEvent(input$karsilastir_button, {
  ...
  shinyjs::html(button_id, "Hesaplanıyor..."); shinyjs::disable(button_id); shinyjs::addClass(button_id, "btn-loading")
  on.exit({ shinyjs::html(button_id, original_html); shinyjs::removeClass(button_id, "btn-loading"); shinyjs::enable(button_id) })
...
observeEvent(input$detay_tablosu_rows_selected, {
  ...
  output$siparis_detay_tablosu <- DT::renderDataTable({ DT::datatable(detay_verisi_final, rownames = FALSE, options = list(pageLength = 10, scrollX = TRUE)) })
})
...
observeEvent(input$firma_karne_tablosu_rows_selected, {
  ...
  output$siparis_detay_tablosu_karne <- DT::renderDataTable({ DT::datatable(detay_verisi_final, rownames = FALSE, options = list(pageLength = 10, scrollX = TRUE)) })
})
...
observeEvent(input$marka_analizi_tablosu_rows_selected, {
  ...
  output$siparis_detay_tablosu_marka <- DT::renderDataTable({ DT::datatable(detay_verisi_final, rownames = FALSE, options = list(pageLength = 10, scrollX = TRUE)) })
})
...
observeEvent(input$sikayet_analizi_tablosu_rows_selected, {
  ...
  output$siparis_detay_tablosu_sikayet <- DT::renderDataTable({ DT::datatable(detay_verisi_final, rownames = FALSE, options = list(pageLength = 10, scrollX = TRUE)) })
})